...
RCSID("$Id: kadm_cli_wrap.c,v 1.24 1998/07/09 23:18:01 assar Exp $");
...
kadm_cli_disconn(void)
{
    closesocket(client_parm.admin_fd);
}
...
kadm_init_link(char *n, char *i, char *r)
{
...
#else
	client_parm.admin_fd = INVALID_SOCKET;
#endif
	...
	client_parm.admin_addr.sin_port = (short)
...
{					/* this connects and sets my_addr */
	...
	if (wkrb_get_our_ip_for_realm(client_parm.krbrlm, &natAddr) == KSUCCESS) {
	    if (natAddr.s_addr != client_parm.my_addr.sin_addr.s_addr)
		client_parm.my_addr.sin_addr = natAddr;
	    }
    }
#endif
...
#else
	if ((stat = des_key_sched(s_k,s_s)) != 0)
		return stat+krb_err_base;
#endif
	return KADM_SUCCESS;
}				       /* This code "works" */
...
kadm_cli_out(u_char *dat, int dat_len, u_char **ret_dat, int *ret_siz)
{
	...
	tmp[0] = (char)((dlen >> 8) & 0xff);
	tmp[1] = (char)(dlen & 0xff);
	      ...
	      int *ret_siz)	/* length of returned info */
{
	...
	*ret_dat = 0;
	...
	if (retdat != KADM_SUCCESS)
	    RET_N_FREE(retdat);
	free(act_st);
#define RET_N_FREE2(r) {/*free(*ret_dat); *ret_dat = 0; */ clear_secrets(); return(r);}
...
int
KADM_LIB_FUNCTION 
kadm_change_pw_plain(unsigned char *newkey, char *password, char **pw_msg)
{
...
int
KADM_LIB_FUNCTION 
kadm_change_pw2(unsigned char *newkey, char *password, char **pw_msg)
{
...
int
KADM_LIB_FUNCTION 
kadm_change_pw(unsigned char *newkey)
{
  char *pw_msg;
...
extern HINSTANCE InstanceId;

const char *
KADM_LIB_FUNCTION 
wkadm_get_error(int errcode)
{
	static char buf[512];

	if ((errcode & 0xff00) == IDS_PREF_KADM) {
		if (LoadString(InstanceId, errcode, buf, sizeof(buf)-1) != 0)
			return buf;
		}

	if ((errcode & 0xff00) == IDS_PREF_KERR)
		return krb_get_err_text(errcode & 0xff);

	sprintf(buf, "Unknown kerberos error: %d (0x%08x)", 
		errcode, errcode);
