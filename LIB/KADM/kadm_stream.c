...
RCSID("$Id: kadm_stream.c,v 1.12 1998/06/09 19:25:11 joda Exp $");
	  ...
	  int loc)		/* offset into the stream for current data */
{
    ...
    if(p == NULL)
	return -1;
    p[loc] = (unsigned char)((dat >> 8) & 0xff);
    p[loc+1] = (unsigned char)(dat & 0xff);
    *st = p;
    return 2;
}
	 ...
	 int loc)		/* offset into the stream for current data */
{
    ...
    if(p == NULL)
	return -1;
    p[loc] = (unsigned char)((dat >> 24) & 0xff);
    p[loc+1] = (unsigned char)((dat >> 16) & 0xff);
    p[loc+2] = (unsigned char)((dat >> 8) & 0xff);
    p[loc+3] = (unsigned char)(dat & 0xff);
    *st = p;
    return 4;
}
...
#ifdef WIN32
static int
strnlen(const char *cp, int maxlen)
{
	register int l;
	
	for (l = 0; (l < maxlen) && (cp[l]); l++);
	return l;
}
	  ...
	  int maxlen)
{
  ...
  *dat = (u_int16_t)((st[loc] << 8) | st[loc + 1]);
  return 2;
}
